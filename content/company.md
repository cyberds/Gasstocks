# Gasstocks Limited — company knowledge base

This document is what the website assistant answers from. Write it for a reader,
not for a machine: plain sentences, no marketing padding, and nothing you would
not be happy to see quoted back to a client.

**Two rules for editing.**

1. The block between the `BEGIN GENERATED` and `END GENERATED` markers is
   produced from `lib/company.ts` and `lib/services.ts` by `npm run sync:company`.
   Do not edit it by hand — your changes will be overwritten. Change the source
   files instead, then re-run the command. This is what keeps the assistant from
   contradicting the website.
2. Everything outside those markers is yours. Add history, values, project
   history, FAQs, anything a customer asks often. The more of it there is, the
   less the assistant needs to fall back to a web search.
...
- **Legal name:** Gasstocks Limited
- **Registration number:** RC359117
- **Commenced business:** 2008
- **Website:** https://gasstocksltd.com
- **Email:** info@gasstocksinc.org
- **Phone (Nigeria):** +234 802 524 0251
- **Phone (United States):** +1 832 382 1221

### Offices

- **Port Harcourt, Nigeria** — 41 Herbert Macauley Street, Port Harcourt, Nigeria
- **Lagos, Nigeria** — Plot 5a Block A10, Admiralty Way, by Admiralty Gate, Lekki Phase 1, Lagos, Nigeria
- **Houston, United States** — 3317 McCulloch Circle, Houston, Texas 77056, United States

## Compliance and certification

This list is exhaustive. Gasstocks holds no certifications beyond those below;
...
> Add real questions here as they come up — this is the highest-value section in
> the file, because it is the part no web search can substitute for.

**Where do you operate?**
We work across Nigeria, with offices in Port Harcourt and Lagos, and a United
...
- If the answer is not in this document, say so and offer to connect the person
  to the team rather than guessing. Never invent a certification, a vessel, a
  project reference, a price or a timeline.
- Gasstocks holds exactly the certifications listed in the compliance section
  above — no others. This matters: clients prequalify us on it.
- For anything commercial — rates, availability, tender responses — hand over to
  a human rather than committing the company to terms.
